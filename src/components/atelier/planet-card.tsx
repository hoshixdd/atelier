  ...
  if (slug === "remnant") {
    return (
      ...
      >
        <button
          type="button"
          onClick={() => flyToShot("/about")}
          onPointerEnter={() => sound.hover()}
          className="grid w-full grid-cols-[5.5rem_1fr] text-left sm:grid-cols-[10rem_1fr]"
        >
          <div className="aspect-square overflow-hidden bg-fog">
            <img src="/cosmos/remnant.png" alt="" className="h-full w-full object-cover" />
          </div>
          <div className="flex flex-col justify-end p-3 sm:p-6">
            <p className="kicker">00 · origin · remnant</p>
            <h2 className="mt-1 font-display text-2xl leading-none sm:text-4xl">Hoshi</h2>
            <p className="mt-2 hidden max-w-sm text-sm leading-relaxed text-bone/90 sm:block">
              The star at the middle of the belt. Come through it.
            </p>
            <p className="mt-3 text-[10px] tracking-[0.28em] text-bone/70 uppercase">
              <span className="md:hidden">Tap to enter</span>
              <span className="hidden md:inline">Enter the star</span>
            </p>
          </div>
        </button>
      </aside>
